...
def main(prompt_str: str) -> str:
    prompt = json.loads(prompt_str)

    prompt_category, prompt_list = list(prompt.items())[0]
    prediction = predict_word(prompt_category, prompt_list)

    return json.dumps(dict(prediction))


if __name__ == "__main__":
    prompt_str = '{"animal": ["dog", "cat"], "fruit": ["apple", "orange"], "animal": ["rabbit", "parrot"]}'
    print(main(prompt_str))